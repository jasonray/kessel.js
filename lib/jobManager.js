...
class JobManager extends EventEmitter {
    constructor(context) {
        ...
        self.context.metrics.gauge('kessel.jobsProcessed').set(0);

        // setInterval(function () {
        //     self.logger.trace(self.context.metrics.gauge('kessel.jobsProcessed').toString());
        //     self.logger.trace(self.context.metrics.meter('kessel.job-timing').toString());
        //     self.logger.trace(self.context.metrics.meter('kessel.job-timing.add').toString());
        //     self.logger.trace(self.context.metrics.meter('kessel.job-timing.multiplication').toString());
        // }, 1000);
    ...
    registerHandler(type, handler) {
        ...
        if (!Utility.isTypeValid(type)) throw new Error('Cannot register handler with invalid type');
    ...
    _checkQueueAndProcessNextJob() {
        ...
        function dequeueCallback(err, jobRequest, commit, rollback) {
            if (_.isEmpty(jobRequest)) {
                ...
                var event1 = self.context.metrics.meter('kessel.job-timing').startEvent();
                var event2 = self.context.metrics.meter('kessel.job-timing.' + jobRequest.type).startEvent();
                const result = self.processSingleJob(jobRequest);
                // self.context.metrics.meter('kessel.job-timing').stop();
                event1.stop();event2.stop();

                self.logger.trace('processed jobRequest:' + jobRequest.id + '; result:' + result);

                self.context.metrics.registry('kessel.jobsProcessed').increment();
    ...
    processSingleJob(jobRequest) {
        ...
        self.logger.trace('job type for job request: ', jobRequest.type);
