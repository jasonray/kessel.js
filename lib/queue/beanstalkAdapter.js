...
BeanstalkAdapter.prototype.dequeue = function (callbackToConsumer) {
    ...
    function isExpired(jobRequest) {
        if (jobRequest.timeout) {
            var expiration = jobRequest.timeout;
            var now = new Date();
            self.logger.trace("now: ", now, "expiration: ", expiration);

            if (now > expiration) {
                self.logger.trace('expired');
                return true;
            } else if (now < expiration) {
                self.logger.trace('i guess not expired');
            } else {
                self.logger.trace('say what??');
            }
        }
        self.logger.trace('not expired');
        return false;
    }
