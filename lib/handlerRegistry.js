...
HandlerRegistry.prototype.registerHandler = function (type, handler) {
    ...
    if (isFunctionBasedHandler(handler)) {
        ...
        return self.registerHandler(type, handler.handle);
    } else if (isModuleIdentifier(handler)) {
        var handlerModule = require(handler);
        if (isObjectBasedHandler(handlerModule)) {
            normalizedHandler = handlerModule.handle;
        } else if (isFunctionBasedHandler(handlerModule)) {
            normalizedHandler = handlerModule;
        }
    }
