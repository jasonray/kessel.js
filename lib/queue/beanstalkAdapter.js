...
class BeanstalkAdapter {
    constructor(specifiedConfig) {
        ...
        function getConfig(config) {
            ...
            if (config && _.isFunction(config.nconf)) {
                ...
                const beanstalkConfig = configUtil.getConfig('beanstalk', config, defaultConfig);
                return beanstalkConfig;
            }

        }

        const config = getConfig(specifiedConfig);
    ...
    enqueue(jobRequest, callback) {
        ...
        const serializedJobRequest = serialize(jobRequest);

        const priority = determinePriority(jobRequest);
        const delay = calculateDelayAsSeconds(jobRequest);
        ...
        function determinePriority(jobRequest) {
            ...
            if (jobRequest) {
                if (_.isNumber(jobRequest.priority)) {
                    if (jobRequest.priority < MIN_PRIORITY) {
                        priority = MIN_PRIORITY;
                    } else if (jobRequest.priority > MAX_PRIORITY) {
        ...
        function calculateDelayAsSeconds(jobRequest) {
            ...
            if (jobRequest) {
                ...
                if (jobRequest.delay) {
                    ...
                    const mDelay = moment(jobRequest.delay);

                    let delayAbsolute = mDelay.diff(mNow, 's');
    ...
    dequeue(callbackToConsumer) {
        ...
        self.client.reserve_with_timeout(self.timeout, function (err, jobID, payload) {
                if (isTimeout(err)) {
                    ...
                    const deserializedJob = deserialize(payload);
        ...
        function isExpired(jobRequest) {
            if (jobRequest.expiration) {
                ...
                const expiration = Date.parse(jobRequest.expiration);
                const now = new Date();
